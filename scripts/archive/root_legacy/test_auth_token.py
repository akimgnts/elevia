#!/usr/bin/env python3
"""
Test de connexion au serveur OAuth France Travail (safe for pytest).
"""
import os
...
def run_token_check(dry_run: bool = False) -> Dict[str, str]:
    data = _load_env()
...
def test_auth_token_env_gate():
    result = run_token_check(dry_run=True)
    assert result["status"] in {"ENV_MISSING", "DRY_RUN", "SUCCESS"}


if __name__ == "__main__":
    print(run_token_check(dry_run=False))
